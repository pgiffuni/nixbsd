{ config, options, lib, pkgs, utils, modules, baseModules, extraModules, nixbsdModulesPath, modulesPath, specialArgs, ... }:
      ...
      let
        scrubbedEval = evalModules {
          ...
          specialArgs = specialArgs // {
            ...
            options = allOpts;
            inherit nixbsdModulesPath modulesPath utils;
          };
        };
    ...
    baseOptionsJSON =
      ...
      in
        pkgs.runCommand "lazy-options.json" {
          ...
          modules =
            "[ "
            + concatMapStringsSep " " (p: ''"${toString p}"'') docModules.lazy
            + " ]";
          passAsFile = [ "modules" ];
        } ''
